...
function spinControls(master,touch=false){
 ...
 const gm=touch?'Inverser : change le sens · Accélérer : piège les joueurs':'<kbd>F</kbd> : inverser · <kbd>Espace</kbd> : accélérer';
 return (master===0?gm:runner)+(humans===2?(master===1?'<br>J2 : <kbd>Maj droite</kbd> inverser · <kbd>Entrée</kbd> accélérer':'<br>J2 : <kbd>←</kbd> / <kbd>→</kbd> à contre-sens'):'');
}
...
function refreshHud(){if(!match)return;const c=match.config,p=match.players[0],gm=match.master===0;const remaining=match.players.filter(p=>p.id!==match.master&&p.alive&&(match.game!=='zombie'||!p.infected)).length;
 ...
 const action=gm?c.masterAction:c.action;$('action-label').textContent=action.toUpperCase();$('action-icon').textContent=match.game==='zombie'?'ϟ':gm?match.game==='spin'?'↻':'◎':'↑';$('action-button').classList.toggle('charging',p.cooldown>0);$('cooldown-label').textContent=p.cooldown>.1?`${p.cooldown.toFixed(1)} s`:touchInput()?'PRÊT':'ESPACE';$('reverse').hidden=!(gm&&match.game==='spin');$('split-labels').hidden=!(humans===2&&match.game==='zombie');
...
function resume(){$('pause-dialog').close();clearInputs();setScreen(pausedFrom);refreshHud();}
...
function loop(now){requestAnimationFrame(loop);const dt=Math.min(.08,Math.max(0,(now-(last||now))/1000));last=now;if(toastUntil&&now>toastUntil){$('toast').classList.remove('visible');toastUntil=0;}if(eventUntil&&now>eventUntil){$('event-toast').classList.remove('visible');eventUntil=0;}
 if(screen==='countdown'){const prev=Math.ceil(count);count-=dt;if(count<=0){setScreen('playing');clearInputs();refreshHud();sound(960,.18);}else if(Math.ceil(count)!==prev){$('countdown').textContent=Math.ceil(count);$('countdown').style.animation='none';void $('countdown').offsetWidth;$('countdown').style.animation='';sound(600,.1);}}
 if(screen==='playing'&&match){accumulator+=dt;let steps=0;while(accumulator>=1/60&&steps++<6){match.step(1/60,getInputs());accumulator-=1/60;}events();if(match.done){finish();accumulator=0;}if(now-hudAt>100){refreshHud();hudAt=now;}}
 else accumulator=0;
 if(screen==='lobby'){lobbyPhase+=dt;const input=getInputs()[0];const p=roster[0];if(!document.querySelector('dialog[open]')){p.x=clamp(p.x+input.x*dt*4.3,-6.5,6.5);p.z=clamp(p.z+input.z*dt*4.3,-5.5,5.5);if(Math.hypot(input.x,input.z)>.05){p.angle=Math.atan2(input.x,input.z);p.walk=(p.walk||0)+dt*12;}else p.walk=0;}}
...
function wire(){
 ...
 $('start-round').onclick=beginCountdown;$('intro-back').onclick=()=>{campaign=false;round=0;lobby();};
 $('pause').onclick=pause;$('resume').onclick=resume;$('quit').onclick=()=>{campaign=false;round=0;lobby();};
 $('next-round').onclick=()=>{if(campaign){if(round===3){round=0;totals.fill(0);campaign=false;lobby();fullParty();}else{round++;lobby();danceUntil=performance.now()+5000;toast(`Intermission · ${GAMES[order[round]].name} t’attend !`);}}else prepare(pendingGame);};
 $('result-home').onclick=()=>{campaign=false;round=0;lobby();};
 $('home').onclick=()=>{campaign=false;round=0;lobby();};
 ...
 document.addEventListener('keyup',e=>keys.delete(e.code));window.addEventListener('blur',()=>{clearInputs();pause();});document.addEventListener('visibilitychange',()=>{if(document.hidden){clearInputs();pause();}});
 controls=bindPointerControls({canvas:$('world'),joystick:$('joystick'),stick:$('stick'),action:$('action-button'),sling:$('sling'),ball:$('sling-ball'),elastic:$('sling-elastic'),
  getState:()=>({playing:screen==='playing'&&!document.querySelector('dialog[open]'),game:match?.game,master:match?.master,humans,alive:match?.players[0].alive,cooldown:match?.players[match.master].cooldown||0,match}),
